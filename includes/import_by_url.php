...
if(isset($_POST['product_url']) && $_POST['product_url']!='')
{
	...
	$nw_url="http://gw.api.alibaba.com/openapi/param2/2/portals.open/api.getPromotionProductDetail/29109?fields=productUrl&productId=".$product_id."";
	
	$data_url = file_get_contents($nw_url);
	$url_dt = json_decode($data_url);
	$url = $url_dt->result->productUrl;
	}
else
{
$url = $_POST['product_url'];
...
foreach($html->find('h1[class="product-name"]') as $a) {
 ...
 $main_img = array();
foreach($html->find('a[class="ui-image-viewer-thumb-frame"] img') as $a) {
 $main_img[] = $a->attr;
}
$product_img = $main_img[0]['src'];
 $var1 = array();
...
array_pop($var_img_ar);
    ...
    $post_id = wp_insert_post($post);
        ...
        if($alib_product_type == "simple")
        {
	     wp_set_object_terms ($post_id,'variable','product_type');
        }
        else
        {
	     wp_set_object_terms ($post_id,'affilate','product_type');
        }
	...
	update_post_meta($post_id, '_stock_status', get_option('alib_avlb_product_status'));
	...
	foreach($size_variations as $size_variation){  
		...
		foreach($color_variations as $color_variation){
			...
			update_post_meta($variation_post_id, '_stock_status', get_option('alib_avlb_product_status'));
...
add_action('wp_ajax_alib_import_by_url', 'alib_import_by_url');
add_action('wp_ajax_nopriv_alib_import_by_url', 'alib_import_by_url');
